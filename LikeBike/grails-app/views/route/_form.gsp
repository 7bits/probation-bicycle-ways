...
<div class="fieldcontain ${hasErrors(bean: routeInstance, field: 'point', 'error')} ">
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="point" action="create"
                    params="['route.id': routeInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'point.label', default: 'Point')])}</g:link>
        </li>
    </ul>
...
<div class="fieldcontain ${hasErrors(bean: routeInstance, field: 'user', 'error')} required">
    ...
    <g:select id="user" name="user.id" from="${likebike.User.list()}" optionKey="id" required=""
              value="${routeInstance?.user?.id}" class="many-to-one"/>
</div>
