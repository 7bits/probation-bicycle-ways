	...
	<body>
		...
		<div id="show-route" class="content scaffold-show" role="main">
			...
			<ol class="property-list route">
				...
				<li class="fieldcontain">
					<span id="point-label" class="property-label"><g:message code="route.point.label" default="Point" /></span>
					
						<g:each in="${routeInstance.point}" var="p">
						<span class="property-value" aria-labelledby="point-label"><g:link controller="point" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
				...
				<li class="fieldcontain">
					<span id="user-label" class="property-label"><g:message code="route.user.label" default="User" /></span>
						...
						<span class="property-value" aria-labelledby="user-label"><g:link controller="user" action="show" id="${routeInstance?.user?.id}">${routeInstance?.user?.encodeAsHTML()}</g:link></span>
